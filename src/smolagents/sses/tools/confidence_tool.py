...
class ConfidenceAssessorTool(Tool):
    ...
    def __call__(self, task_description, domain=None):
        ...
        assessment = {
            ...
            "level": "high" if confidence_score > 0.85 else 
    ...
    def _analyze_factors(self, task_description, domain=None):
        ...
        specific_terms = sum(1 for term in ['calculate', 'find', 'analyze', 'compare', 'explain'] 
                           if term in task_description.lower())
        
        factors["task_clarity"] = min(1.0, 0.5 + 0.1 * specific_terms + 
                                   (0.4 if word_count > 10 else 0.2) -
                                   (0.1 if question_marks > 2 else 0))
        
        # Reference quality
        url_count = len(re.findall(r'https?://\S+', task_description))
        reference_terms = sum(1 for term in ['reference', 'according to', 'as stated in', 'cited']
                           if term in task_description.lower())
        ...
        if domain:
            ...
            if domain in domain_specific_terms:
                term_count = sum(1 for term in domain_specific_terms[domain]
                               if term in task_description.lower())
